...
if (isset($_POST['id_cuestionario'], $_POST['respuesta'])) {
    ...
    if (isset($_SESSION['id_usuario'])) {
        $idUsuario = intval($_SESSION['id_usuario']); // Obtener el ID del usuario logueado desde la sesión
        ...
        if (is_array($_POST['respuesta'])) {
            ...
            foreach ($respuestas as $idPregunta => $respuesta) {
                ...
                if ($resultadoPreguntaCuestionario->num_rows > 0) {
                    ...
                    $idPreguntaCuestionario = intval($rowPreguntaCuestionario['id']);

                    // Escapar el input del usuario para evitar inyección SQL
                    $respuesta = $conn->real_escape_string($respuesta);

                    // Guardar la respuesta en la base de datos incluyendo `id_pregunta`
                    $sqlRespuesta = "
                    INSERT INTO respuestaestudiante (id_estudiante, id_preguntacuestionario, id_cuestionario, id_pregunta, respuesta1)
                    VALUES ('$idUsuario', '$idPreguntaCuestionario', '$idCuestionario', '$idPregunta', '$respuesta')";
